...
namespace Commanigy.Iquomi.Web {
	...
	/// 
	/// </summary>
	public partial class UcPackage : System.Web.UI.UserControl {
		...
		private DbPackage dataItem;

		public DbPackage DataItem {
			get {
				return dataItem;
			}
		...
		protected void ObjectSource_Inserting(object sender, ObjectDataSourceMethodEventArgs e) {
			DbPackage a = e.InputParameters[0] as DbPackage;
			a.State = DbPackage.StateNew;
		}
		
		protected void ObjectSource_Inserted(object sender, ObjectDataSourceStatusEventArgs e) {
			dataItem = (DbPackage)e.ReturnValue;
			if (dataItem != null && dataItem.Id > 0) {
				(Page as IPageNotification).Success(/*dataItem*/);
			}
			else {
		...
		protected void ObjectSource_Updating(object sender, ObjectDataSourceMethodEventArgs e) {
			DbPackage v = e.InputParameters[0] as DbPackage;
			v.Id = dataItem.Id;
			v.ServiceId = dataItem.ServiceId;
			v.State = dataItem.State;
		}
