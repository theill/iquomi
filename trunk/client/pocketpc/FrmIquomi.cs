...
// 
		...
		private void menuItem6_Click(object sender, System.EventArgs e) {
			...
			req.XpQueries = new XpQueryType[] { query };
			
			QueryResponseType res = myService.Query(req);
			if (res != null && res.XpQueryResponses != null && res.XpQueryResponses[0].Status == ResponseStatus.Success) {
				if (res.XpQueryResponses[0].Any.Length == 0) {
					return;
				}
				
				serviceType[] services = (serviceType[])ServiceUtils.GetObject(
					typeof(serviceType), 
					res.XpQueryResponses[0].Any
					);
